...
export const api = {
  ...
  async getCategories(subdomain: string = DEFAULT_SUBDOMAIN): Promise<string[]> {
    const res = await fetch(`${API_BASE_URL}/storefront/public/${subdomain}/categories`, {
  ...
  async getStoreInfo(subdomain: string = DEFAULT_SUBDOMAIN): Promise<StorefrontInfo> {
    const res = await fetch(`${API_BASE_URL}/storefront/public/${subdomain}/info`, {
      next: { revalidate: 3600 },
    });
  ...
  async createOrder(subdomain: string = DEFAULT_SUBDOMAIN, orderData: any) {
    const res = await fetch(`${API_BASE_URL}/storefront/${subdomain}/checkout`, {
      method: 'POST',
